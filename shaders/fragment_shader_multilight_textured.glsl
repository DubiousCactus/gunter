...
vec3 computeDirectionalLight(DirectionalLight light, vec3 normal,
    ...
    vec3 diffuse =
        max(dot(normal, light_dir), 0.0) *
            (u_has_diffuse_texture ? texture(u_material.diffuse, io_text_coords).rgb : vec3(1.0)) *
            light.diffuse;
    vec3 ambient =
        light.ambient * (u_has_diffuse_texture
            ? texture(u_material.diffuse, io_text_coords).rgb : vec3(1));
    vec3 specular =
        pow(max(dot(reflect(-light_dir, normal), view_dir), 0.0),
            u_material.shininess) *
            (u_has_specular_texture ? texture(u_material.specular, io_text_coords).r : 0.0) *
            light.specular;
...
vec3 computePointLight(PointLight light, vec3 normal, vec3 frag_pos,
    ...
    vec3 ambient =
        light.ambient * (u_has_diffuse_texture
            ? texture(u_material.diffuse, io_text_coords).rgb : vec3(1.0));
    vec3 specular =
        pow(max(dot(reflect(-light_dir, normal), view_dir), 0.0),
            u_material.shininess) *
            (u_has_specular_texture ? texture(u_material.specular, io_text_coords).r : 0.0) *
...
vec3 computeSpotLight(SpotLight light, vec3 normal, vec3 frag_pos,
    ...
    vec3 diffuse =
        max(dot(normal, incoming_light_dir), 0.0) *
            (u_has_diffuse_texture ? texture(u_material.diffuse, io_text_coords).rgb : vec3(1)) *
            light.diffuse;
    vec3 specular =
...
void main() {
    if (u_is_source) {
        ...
        float alpha = u_has_diffuse_texture
            ? texture(u_material.diffuse, io_text_coords).a : 1.0;
