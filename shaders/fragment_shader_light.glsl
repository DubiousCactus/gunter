...
struct Material {
  ...
  vec3 diffuse;    // Color of the surface under diffuse lighting, typically the
                   // ambient color
...
void main() {
  if (u_is_source) {
    ...
    vec3 ambient = u_material.ambient * u_dir_light.ambient;
    ...
    vec3 specular = pow(max(dot(reflect(-light_dir, normal), view_dir), 0.0),
                    ...
                    u_material.specular * u_dir_light.specular;
